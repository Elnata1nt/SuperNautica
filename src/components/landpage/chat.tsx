...
export default function Chat() {
  ...
  useEffect(() => {
    if (countdown !== null && countdown > 0) {
      ...
      redirectToWhatsApp(lastUserMessage ? lastUserMessage.text : "Olá, gostaria de mais informações.");
      setCountdown(null);
    }
  }, [countdown, messages]);
  ...
  const handleSend = (text: string = input, isQuickReply: boolean = false) => {
    if (text.trim()) {
      ...
      setTimeout(() => {
        setIsTyping(false);
        const botReply = isQuickReply
          ? `Obrigado por selecionar uma resposta rápida: "${text}". Você será redirecionado para o WhatsApp em 5 segundos com sua mensagem.`
          : `Obrigado por sua mensagem. Você será redirecionado para o WhatsApp em 5 segundos com o seguinte texto: "${text}"`;
